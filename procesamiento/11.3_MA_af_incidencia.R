# OJO: Verifica el nombre del source si cambiaste el nombre del script anterior.
source("procesamiento/10.3_KM_Cox_af_incidencia.R", encoding = "UTF-8")
...
# Extraer AF de cada ENS y unir (USANDO LAS TABLAS DE INCIDENCIA)
af_2009  <- get_af(resumen_2009_incidencia_af, "ENS 2009-2010")
af_2016  <- get_af(resumen_2016_incidencia_af, "ENS 2016-2017")
...
forest(m_af, backtransf = TRUE,
       xlab = "Hazard Ratio",
       main = "Incidencia de Cáncer y AF — Modelo completo",
       leftlabs = c("Estudio"),
       rightlabs = c("HR (IC95%)"),
...
png("output/graphs/incidencia/Metaanalisis_AF_Incidencia_ModeloCompleto.png", 
...
sx_2009 <- get_sexo(resumen_2009_incidencia_af, "ENS 2009-2010")
sx_2016 <- get_sexo(resumen_2016_incidencia_af, "ENS 2016-2017")
...
forest(m_sexo, backtransf = TRUE, xlab = "Hazard Ratio",
       main = "Incidencia: Sexo (Mujer vs Hombre) — Modelo completo",
       leftlabs = "Estudio", rightlabs = "HR (IC95%)",
       smlab = "Modelo de efectos aleatorios")

# 1. Abrimos el archivo PNG indicando la ruta y el tamaño
png("output/graphs/Metaanalisis_AF_Incidencia_solo_sexo.png", 
    width = 10, height = 5, units = "in", res = 300)
...
af_crudo_2009  <- get_af_crudo(resumen_2009_incidencia_af, "ENS 2009-2010")
af_crudo_2016  <- get_af_crudo(resumen_2016_incidencia_af, "ENS 2016-2017")
...
forest(m_af_crudo, backtransf = TRUE,
       xlab = "Hazard Ratio",
       main = "Incidencia Cáncer y AF — Modelo Crudo",
       leftlabs = c("Estudio"),
       rightlabs = c("HR (IC95%)"),
       smlab = "Modelo de efectos aleatorios")
...
# Guardar gráfico final (descomentar para guardar físicamente)
# 1. Abrimos el archivo PNG indicando la ruta y el tamaño
png("output/graphs/incidencia/Metaanalisis_AF_Incidencia_Modelocrudo.png", 
    width = 10, height = 5, units = "in", res = 300)
...
af_edadsexo_2009  <- get_edadsexo(resumen_2009_incidencia_af, "ENS 2009-2010")
af_edadsexo_2016  <- get_edadsexo(resumen_2016_incidencia_af, "ENS 2016-2017")
...
forest(m_af_edadsexo, backtransf = TRUE,
       xlab = "Hazard Ratio",
       main = "Incidencia Cáncer y AF — Modelo Edad+Sexo",
       leftlabs = c("Estudio"),
       rightlabs = c("HR (IC95%)"),
       smlab = "Modelo de efectos aleatorios")
...
# Guardar gráfico final (descomentar para guardar físicamente)
# 1. Abrimos el archivo PNG indicando la ruta y el tamaño
png("output/graphs/incidencia/Metaanalisis_AF_Incidencia_Modeloedadsexo.png", 
    width = 10, height = 5, units = "in", res = 300)

...
#Guardar gráfico
ggsave("output/graphs/incidencia/KM_AF_Incidencia_2009.png", width = 10, height = 6, dpi = 300)
...
ens2016_final <- ens2016_final %>%
  mutate(
    tiempo_total = dias_transcurridosfinal / 365.25, # Nueva variable
    evento_total = if_else(incidencia == 1, 1, 0),   # Nueva variable
    evento_label = factor(evento_total,
                          levels = c(0, 1),
                          labels = c("No es caso nuevo", "Caso nuevo de cáncer"))
  )
...
# Limpiar los nombres de los grupos
df_km_2016$grupo <- gsub("af_cancer_binaria=", "", df_km_2016$grupo)

# 3. Generar el gráfico aesthetic
...
# Sobrevida a los 5 años (Grupo sin AF)
km_fit_2016[[1]]$surv[max(which(km_fit_2016[[1]]$time <= 5))]

# Sobrevida a los 10 años (Grupo sin AF)
km_fit_2016[[1]]$surv[max(which(km_fit_2016[[1]]$time <= 10))]

#Guardar gráfico
#ggsave("output/graphs/incidencia/KM_AF_Incidencia_2016.png", width = 10, height = 6, dpi = 300)
...
resumen_2009_incidencia_af <- bind_rows(
...
# Mostrar tabla
kable(resumen_2009_incidencia_af, digits = 3, caption = "Modelos de Cox para incidencia de cáncer")

# Generamos la tabla con kable y la guardamos en un objeto
tabla_md <- kable(resumen_2009_incidencia_af, digits = 3, caption = "Modelos de Cox para incidencia de cáncer")

# Usamos writeLines para exportar ese objeto a un archivo .md
#writeLines(as.character(tabla_md), "output/tables/AF_cancer/tabla_cox_resumen_af_incidencia_2009.md")

# 4️⃣ Forest plot para HR deaf_cancer_binaria
resumen_hr <- resumen_2009_incidencia_af %>%
...
ggplot(resumen_hr, aes(x = modelo, y = HR, ymin = IC_inf, ymax = IC_sup)) +
  ...
  labs(
    ...
    title = "Forest plot: Efecto de AF cáncer sobre incidencia de cáncer",
...
df_forest_2009 <- resumen_2009_incidencia_af %>%
...
p_forest_2009 <- ggplot(df_forest_2009, aes(x = HR, y = item, color = variable)) +
  ...
  labs(
    ...
    y = NULL,
    color = NULL,
    title = "Forest plot — HR de antecedentes familiares y sexo (Incidencia - ENS 2009)",
...
#Guardar gráfico
#ggsave("output/graphs/incidencia/FP_HR_AF_Incidencia_2009.png", width = 10, height = 6, dpi = 300)

#=========================================================
#### COX con muestra expandida 2016 ####
#=========================================================

# 1. Actualizar el diseño de encuesta
...
resumen_2016_incidencia_af <- bind_rows(
...
kable(resumen_2016_incidencia_af, digits = 3, caption = "Modelos de Cox para incidencia de cáncer - ENS 2016")

# Guardar la tabla en formato Markdown
tabla_md_2016 <- kable(resumen_2016_incidencia_af, digits = 3, caption = "Modelos de Cox para incidencia de cáncer - ENS 2016")

#writeLines(as.character(tabla_md_2016), "output/tables/AF_cancer/tabla_cox_resumen_af_incidencia_2016.md")


# 4. Forest plot para HR de af_cancer_binaria (Mejorado)
resumen_hr_2016 <- resumen_2016_incidencia_af %>%
  filter(term == "af_cancer_binariaYes") %>%
  mutate(modelo = factor(modelo, levels = c("Crudo", "Edad", "Edad+Sexo", "Modelo completo")))
...
ggplot(resumen_hr_2016, aes(x = modelo, y = HR, ymin = IC_inf, ymax = IC_sup)) +
  ...
  labs(
    x = "",
    y = "HR de AF cáncer (IC 95%)",
    title = "Forest plot: Efecto de AF cáncer sobre incidencia de cáncer (ENS 2016)",
...
df_forest_2016 <- resumen_2016_incidencia_af %>%
  filter(term %in% c("af_cancer_binariaYes", "sexoFemale")) %>%
  mutate(
    variable = case_when(
      term == "af_cancer_binariaYes" ~ "AF familiar (Con AF vs Sin AF)",
...
p_forest_2016 <- ggplot(df_forest_2016, aes(x = HR, y = item, color = variable)) +
  ...
  labs(
    ...
    y = NULL,
    color = NULL,
    title = "Forest plot — HR de antecedentes familiares y sexo (Incidencia - ENS 2016)",
    subtitle = "Modelos de Cox ponderados por diseño muestral",
    caption = "Barras = IC95%. Línea punteada = HR = 1"
  ) +
  theme_classic(base_size = 12) +
...
#ggsave("output/graphs/incidencia/FP_HR_AF_Incidencia_2016.png", plot = p_forest_2016, width = 10, height = 6, dpi = 300)

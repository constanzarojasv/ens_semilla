...
ens2003_final$egresoydefuncion<-as.factor(ens2003_final$egresoydefuncion)
ens2003_final$Depresion_1_AP <- as.factor(ens2003_final$Depresion_1_AP)

# Preparar variables
ens2003_final <- ens2003_final %>%
  mutate(
    tiempo_totalfinal = dias_transcurridosfinal / 365.25,
    evento_total = if_else(egresosydefuncion == 1, 1, 0),
    evento_label = factor(evento_total,
                          levels = c(0, 1),
                          labels = c("No egresados ni muerto por cáncer", "Egresados o muerto por cáncer"))
  )
...
km_fit <- svykm(Surv(tiempo_totalfinal, evento_total) ~ Depresion_1_AP, 
